...
// @ts-ignore
const input_js_1 = require("enquirer/lib/prompts/input.js");
// @ts-ignore
const base_1 = require("@scure/base");
const btc_signer_1 = require("@scure/btc-signer");
// @ts-ignore
const select_js_1 = require("enquirer/lib/prompts/select.js");
const index_ts_1 = require("./index.js");
/*

*/
const { BROTLI_MODE_GENERIC: B_GENR, BROTLI_MODE_TEXT: B_TEXT, BROTLI_MODE_FONT } = node_zlib_1.constants;
// Max script limit.
...
const customScripts = [index_ts_1.OutOrdinalReveal];
...
const validateTxid = (s) => {
    try {
        const txid = base_1.hex.decode(s);
        if (txid.length !== 32)
...
const validateIndex = (s) => {
    const n = Number.parseInt(s);
    // Index is U32LE
    const matches = s === `${n}` && Number.isInteger(n) && 0 <= n && n < 2 ** 32;
    return matches || `Number must be between 0 and ${2 ** 32}`;
};
const validateAmount = (s) => {
    try {
        const n = btc_signer_1.Decimal.decode(s);
        if (n <= 0)
            return `amount should be bigger than zero`;
        return true;
    }
    catch (e) {
        return `${e}`;
...
const select = async (message, choices) => {
    try {
        return await new select_js_1.default({ message, choices }).run();
    }
    catch (e) {
        process.exit(); // ctrl+c
    }
...
async function input(message, validate) {
    ...
    try {
        return await new input_js_1.default(opts).run();
    }
    catch (e) {
        process.exit(); // ctrl+c
    }
...
const formatBytes = (n) => `${magenta}${bfmt.format(n)}${reset}`;
const formatSatoshi = (n) => `${magenta}${n}${reset} satoshi (${magenta}${btc_signer_1.Decimal.encode(n)}${reset} BTC)`;
const formatAddress = (s) => `${green}${s}${reset}`;
// /UI
...
for (const [type, brotliMode, exts] of contentTypeTable) {
    for (const ext of exts)
        contentType.set(ext, [type, brotliMode]);
}
const NETWORKS = {
    mainnet: { ...btc_signer_1.NETWORK, name: `${red}mainnet${reset}` },
    testnet: { ...btc_signer_1.TEST_NETWORK, name: `testnet` },
};
const usage = (err) => {
    if (err)
        console.error(`${red}ERROR${reset}: ${err}`);
    console.log(`Usage: ${green}ord-cli${reset} [--net ${Object.keys(NETWORKS).join('|')}] [--priv key] [--recovery key] [--compress=on|off] [--fee 10.1] [--addr address] <path>`);
    console.log(HELP_TEXT);
    process.exit();
};
async function getNetwork(opts) {
    if (!opts.net)
        opts.net = await (0, exports.select)('Network', ['testnet', 'mainnet']);
...
function getKeys(net, opts) {
    ...
    for (const name in KEYS) {
        ...
        if (opts[name])
            res[name] = (0, btc_signer_1.WIF)(net).decode(opts.priv);
        else {
            res[name] = btc_signer_1.utils.randomPrivateKeyBytes();
            console.log(`${KEYS[name]} private key: ${red}${(0, btc_signer_1.WIF)(net).encode(res[name])}${reset}`);
        }
        if (res[name].length !== 32) {
            return usage(`wrong ${KEYS[name].toLowerCase()} private key, expected 32-bytes, got ${res[name].length}`);
        }
    }
    console.log(`${bold}Important:${reset} if there is an issue with reveal transaction, you will need these keys to refund sent coins`);
    return res;
}
function getInscription(filePath, opts) {
    const stat = (0, node_fs_1.lstatSync)(filePath);
    if (!stat.isFile())
        return usage(`path is not file "${filePath}"`);
    ...
    const info = [];
    info.push(`mime=${mime}`);
    let data = Uint8Array.from((0, node_fs_1.readFileSync)(filePath, null));
    let inscription = { tags: { contentType: mime }, body: data };
    info.push(`size=${formatBytes(data.length)}`);
    if (!opts.compress || opts.compress !== 'off') {
        const compressed = (0, node_zlib_1.brotliCompressSync)(data, {
            params: {
                [node_zlib_1.constants.BROTLI_PARAM_MODE]: brotliMode,
                [node_zlib_1.constants.BROTLI_PARAM_QUALITY]: node_zlib_1.constants.BROTLI_MAX_QUALITY,
                [node_zlib_1.constants.BROTLI_PARAM_SIZE_HINT]: data.length,
            },
        });
        ...
        info.push(`${red}uncompressed${reset}`); // notify user that compression disabled
...
async function getFee(opts) {
    let fee = opts.fee;
    if (!fee)
        fee = await input(`Network fee (in satoshi)`, validateFloat);
    if (validateFloat(fee) !== true)
        return usage(`wrong fee=${fee}`);
    return parseFloat(fee);
}
async function getAddr(net, opts) {
    let address = opts.addr;
    const validate = (s) => {
        try {
            (0, btc_signer_1.Address)(net).decode(s);
            return true;
        }
        catch (e) {
            return `${e}`;
    ...
    if (validate(address) !== true)
        return usage(`wrong address=${address}`);
    return address;
}
function getPayment(privKey, recovery, inscription, net) {
    const pubKey = btc_signer_1.utils.pubSchnorr(privKey);
    const recoveryPub = btc_signer_1.utils.pubSchnorr(recovery);
    const rev = (0, index_ts_1.p2tr_ord_reveal)(pubKey, [inscription]);
    return (0, btc_signer_1.p2tr)(recoveryPub, rev, net, false, customScripts);
}
function getTransaction(privKey, addr, payment, net, txid, index, amount, fee) {
    const tx = new btc_signer_1.Transaction({ customScripts });
    tx.addInput({
        ...payment,
        txid,
        index,
...
async function main() {
    try {
        ...
        // @ts-ignore
        if (import.meta.url !== `file://${(0, node_fs_1.realpathSync)(argv[1])}`)
            return; // ESM is broken.
        if (argv.length < 3)
            return usage('Wrong argument count'); // node script file
        ...
        if (dummyTx.weight >= MAX_STANDARD_TX_WEIGHT) {
            return usage(`File is too big: reveal transaction weight (${dummyTx.weight}) is higher than limit (${MAX_STANDARD_TX_WEIGHT})`);
        }
        ...
        console.log(`Created. Please send at least ${formatSatoshi(minAmount)} to ${formatAddress(payment.address)}`);
        // Ask for UTXO
        ...
        const amount = btc_signer_1.Decimal.decode(await input('Amount', validateAmount));
        ...
        console.log(base_1.hex.encode(tx.extract()));
        console.log(`Please broadcast this transaction to reveal inscription and transfer to your address (${formatAddress(addr)})`);
        console.log(`${bold}Important:${reset} please freeze this UTXO in your wallet when received to avoid sending inscription as fees for other transactions.`);
    }
    catch (e) {
        return usage(e);
    }
