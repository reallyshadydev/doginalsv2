...
import { Address, Decimal, NETWORK, TEST_NETWORK, Transaction, WIF, p2tr, utils, } from '@scure/btc-signer';
// @ts-ignore
import Select from 'enquirer/lib/prompts/select.js';
import { OutOrdinalReveal, p2tr_ord_reveal } from "./index.js";
/*
...
export async function input(message, validate) {
    let opts = { message };
    if (validate)
...
const formatSatoshi = (n) => `${magenta}${n}${reset} satoshi (${magenta}${Decimal.encode(n)}${reset} BTC)`;
...
async function getNetwork(opts) {
    if (!opts.net)
        opts.net = await select('Network', ['testnet', 'mainnet']);
...
function getKeys(net, opts) {
    ...
    for (const name in KEYS) {
        ...
        if (opts[name])
            res[name] = WIF(net).decode(opts.priv);
        else {
...
async function main() {
    try {
        ...
        // @ts-ignore
        if (import.meta.url !== `file://${realpathSync(argv[1])}`)
