...
import { MAX_SCRIPT_BYTE_LENGTH, Script, utils, } from '@scure/btc-signer';
...
const TagCoder = {
    encode(from) {
        ...
        for (const field in tmp) {
            ...
            res[field] = TagCoders[field].decode(utils.concatBytes(...tmp[field]));
        }
        return res;
    },
    decode(to) {
...
export function parseInscriptions(script, strict = false) {
    if (strict && (!utils.isBytes(script[0]) || script[0].length !== 32))
        return;
 ...
 */
export function p2tr_ord_reveal(pubkey, inscriptions) {
    return {
        type: 'tr',
